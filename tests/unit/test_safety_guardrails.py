...
class TestSafetyGuard:
    ...
    def test_detects_os_system(self, guard: SafetyGuard) -> None:
        violations = guard.check_python(
            'import os\nos.system("rm -rf /")'
        )
        assert len(violations) > 0

    def test_detects_shutil_rmtree_root(
        self, guard: SafetyGuard
    ) -> None:
        violations = guard.check_python(
            'import shutil\nshutil.rmtree("/")'
    ...
    def test_allows_trusted_domain(
        self, guard: SafetyGuard
    ) -> None:
        violations = guard.check_url(
            "https://arxiv.org/abs/2301.00001"
        )
        assert len(violations) == 0

    def test_blocks_untrusted_domain(
        self, guard: SafetyGuard
    ) -> None:
    ...
    def test_allows_subdomain(self, guard: SafetyGuard) -> None:
        violations = guard.check_url(
            "https://api.huggingface.co/datasets"
        )
        assert len(violations) == 0
    ...
    def test_check_tool_call_safe_python(
        self, guard: SafetyGuard
    ) -> None:
        violations = guard.check_tool_call(
    ...
    def test_check_tool_call_js_child_process(
        self, guard: SafetyGuard
    ) -> None:
        violations = guard.check_tool_call(
            "js_exec",
    ...
    def test_check_tool_call_dataset_fetch_url(
        self, guard: SafetyGuard
    ) -> None:
        violations = guard.check_tool_call(
            "dataset_fetch",
    ...
    def test_should_not_block_empty(
        self, guard: SafetyGuard
    ) -> None:
